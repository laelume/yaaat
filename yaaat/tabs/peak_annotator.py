...
class PeakAnnotator:
    ...
    def update_prominence(self):
        ...
        if self.show_auto_peaks.get() and self.y is not None:
            self.update_display(recompute_all=False)  # Redetect peaks with new prominence
    ...
    def load_current_file(self):
        ...
        self.changes_made = False
        self.zoom_stack = []
        self.update_display(recompute_all=True)
    ...
    def auto_detect_peaks(self):
        ...
        self.update_display(recompute_all=False)
    ...
    def update_display(self, recompute_all=False, **kwargs):
            """Update the display with spectrogram, PSD, and peak annotations"""
            
            if 'recompute_spec' in kwargs and kwargs['recompute_spec'] is not None:
                recompute = kwargs['recompute_spec']
            try:
                if self.y is None:
                    return
                
                if recompute_all or self.spec_image is None:
                    # Full redraw
                    self.ax.clear()

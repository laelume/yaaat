...
class ChangepointAnnotator:
    ...
    def __init__(self, root):
        ...
        # self.syllables = []  #  REPLACED NOMENCLATURE SYLLABLES >> CONTOURS
    ...
    def finish_lasso_selection(self):
        ...
        try: 
            ...
            self.rebuild_annotations()  # This will assign onset/offset annotations properly
    ...
    def load_directory(self):
        ...
        # self.annotation_dir = daily_dir / dataset_name
    ...
    def load_current_file(self):
        ...
        # Update filename_label (annotation_file previously defined above)
        print(f"DEBUG: Setting filename_label to: {annotation_file.name}")
        ...
        if is_skipped:
            ...
            for i in range(self.current_file_idx + 1):
                ...
                if fname_prefix and fname_prefix != '.':
                    lbl_file = self.annotation_dir / f"{fname_prefix}_{audio_f.stem}_changepoint_annotations.json"
                else:
                    lbl_file = self.annotation_dir / f"{audio_f.stem}_changepoint_annotations.json"
            ...
            display_text = f"⊘ SKIPPED ({skipped_count}/{self.total_skipped_files}): {annotation_file.name}"
            if skip_reason:
                display_text += f"\nReason: {skip_reason}"
            self.filename_label.config(text=display_text, foreground='orange')
        elif self.file_was_annotated:
            self.filename_label.config(text=f"✓ {annotation_file}", foreground='green')
        else:
            self.filename_label.config(text=f"→ {annotation_file}", foreground='blue')
    ...
    def load_custom_annotation_dir(self):
        ...
        custom_dir = filedialog.askdirectory(
            ...
            initialdir=str(self.annotation_dir.parent) if self.annotation_dir else None
        )
        ...
        for json_file in json_files:
            dest_file = self.annotation_dir / json_file.name
        ...
        messagebox.showinfo("Annotations Copied", 
            ...
            f"From: {custom_dir}\n"
            f"To: {self.annotation_dir}")
        
        print(f"✓ Copied {copied_count} files from {custom_dir} to {self.annotation_dir}")
    ...
    def save_annotations(self):
        ...
        try:
            ...
            if self.annotation_dir is None:
                print("ERROR: annotation_dir is None")
                return
            
            print(f"Label dir: {self.annotation_dir}")
            print(f"Label dir exists: {self.annotation_dir.exists()}")
